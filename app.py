...
# =============================================================================
@st.cache_data(show_spinner="Cargando Censo y CASEN...")
def load_data():
    df_viv = pl.scan_csv('Data/viviendas_censo2024.csv', separator=';')
    
    df_viv = df_viv.with_columns([
        (pl.col('p4a_mat_paredes').is_in([5, 6]) | pl.col('p4b_mat_techo').is_in([7, 8]) | pl.col('p4c_mat_piso').is_in([4, 5])).cast(pl.Int8).alias('mat_precaria'),
        (pl.col('p6_fuente_agua') != 1).cast(pl.Int8).alias('deficit_agua'),
        (pl.col('p8_serv_hig').is_in([4,5,6,7,8,9])).cast(pl.Int8).alias('deficit_saneamiento'),
        (pl.col('p9_fuente_elect').is_in([5, 6])).cast(pl.Int8).alias('deficit_elect')
    ])
    
    agg_viv = (
        df_viv.group_by(['region', 'comuna', 'area'])
        .agg([
            pl.count('p3b_estado_ocupacion').alias('Total_Viviendas'),
            pl.col('p3b_estado_ocupacion').is_in([1, 2]).sum().alias('Ocupadas'),
            (pl.col('p3b_estado_ocupacion') == 8).sum().alias('De_Vacaciones'),
            pl.col('p3b_estado_ocupacion').is_in([6, 7, 9, 10]).sum().alias('Desocupadas'),
            pl.when(pl.col('p3b_estado_ocupacion') == 1).then(1).otherwise(0).sum().alias('Viviendas_Ocup_Presentes'),
            pl.when((pl.col('p3b_estado_ocupacion') == 1) & (pl.col('mat_precaria') == 1)).then(1).otherwise(0).sum().alias('Viviendas_Precarias'),
            pl.col('deficit_agua').sum().alias('Deficit_Agua'),
            pl.col('deficit_saneamiento').sum().alias('Deficit_Saneamiento'),
            pl.col('deficit_elect').sum().alias('Deficit_Elect')
        ]).collect().to_pandas()
    )
    
    agg_tipo = (
        df_viv.filter(pl.col('p3b_estado_ocupacion') == 1)
        .group_by('p2_tipo_vivienda')
        .agg([pl.col('mat_precaria').count().alias('n'), pl.col('mat_precaria').mean().alias('tasa_precaria')])
        .collect().to_pandas()
    )
    agg_tipo['tasa_precaria'] *= 100
    agg_tipo['tipo_name'] = agg_tipo['p2_tipo_vivienda'].map(TIPO_VIV_NAMES)
    agg_tipo = agg_tipo.sort_values('n', ascending=False)
    
    agg_sunburst = (
        df_viv.group_by(['area', 'p2_tipo_vivienda', 'p3b_estado_ocupacion'])
        .agg(pl.len().alias('count'))
        .collect().to_pandas()
    )
    
    agg_per = (
        pl.scan_csv('Data/personas_censo2024.csv', separator=';')
        .select(['region', 'comuna', 'edad'])
        .group_by(['region', 'comuna'])
        .agg([(pl.col('edad') >= 65).sum().alias('mayores_65'), (pl.col('edad') < 15).sum().alias('menores_15')])
        .with_columns((pl.col('mayores_65') / pl.col('menores_15') * 100).alias('indice_envejecimiento'))
        .collect().to_pandas()
    )
    
    df_merged = pd.merge(agg_viv, agg_per, on=['region', 'comuna'], how='left')
    df_merged['region_name'] = df_merged['region'].map(REG_NAMES)
    df_merged['region_name'] = pd.Categorical(df_merged['region_name'], categories=ORDEN_REGIONES, ordered=True)
    df_merged['comuna_name'] = df_merged['comuna'].map(COMUNAS_DICT).fillna(df_merged['comuna'].astype(str))
